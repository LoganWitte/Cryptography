...
int main() {
    // Put test cases here.
    unsigned long long input = 0x3F6BD297ULL;
    const int perm[] = {32, 1, 2, 3, 4, 5,4, 5, 6, 7, 8, 9,8, 9, 10, 11, 12, 13,12, 13, 14, 15, 16, 17,16, 17, 18, 19, 20, 21,20, 21, 22, 23, 24, 25,24, 25, 26, 27, 28, 29,    28, 29, 30, 31, 32, 1};
    printBits(applyPerm(input, perm));
    return 0;
}
...
unsigned long long applyPerm(unsigned long long block, const int perm[]) {
    ...
    for(int i = 0; i < 48; i++) {
        ...
        int srcIndex = 32 - perm[i]; //perm[i] is 1-32, need 0-31
